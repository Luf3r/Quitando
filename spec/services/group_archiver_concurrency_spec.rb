...
RSpec.describe GroupArchiver, :non_transactional do
  self.use_transactional_tests = false

  it "faz criação de despesa falhar quando o arquivamento vence o lock do grupo" do
    group = create(:group)
    owner = create(:user)
    member = create(:user)
    ...
    first = Thread.new do
      ActiveRecord::Base.connection_pool.with_connection do |connection|
        connection.transaction do
          ...
          locked << true
          release.pop
          results << described_class.call(group_id: group.id, actor_user_id: owner.id)
        end
      end
    ...
    state = Timeout.timeout(5) { locked.pop }
    raise state if state.is_a?(StandardError)

    second = Thread.new do
      ActiveRecord::Base.connection_pool.with_connection do
        results << ExpenseCreator.call(
          group_id: group.id, created_by_user_id: owner.id, paid_by_user_id: owner.id,
          description: "Depois do arquivo", occurred_on: Date.current, amount_text: "2,00",
    ...
    expect(values.count { |value| value.is_a?(Group) }).to eq(1)
    expect(values.grep(ExpenseCreator::InvalidExpense)).to contain_exactly(have_attributes(message: "grupo arquivado"))
