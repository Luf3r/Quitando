...
RSpec.describe "Conta pessoal" do
  ...
  it "mostra edição de nome, e-mail e senha sem oferecer exclusão" do
    user = create(:user, name: "Ana Vitória", email: "ana@example.com")
    sign_in_as user

    get account_path

    expect(response).to have_http_status(:ok)
    expect(response.body).to include("Ana Vitória")
  ...
  it "atualiza o nome normalizado mediante senha atual" do
    user = create(:user, name: "Ana", email: "ana@example.com", password: "senha-segura")
    sign_in_as user
    ...
    expect(user.reload.name).to eq("Ana Vitória")
  end
  ...
  it "bloqueia no backend alterações das credenciais de uma conta demo" do
    user = create(:user, :demo_account, name: "Ana", email: "ana-demo-#{SecureRandom.hex(4)}@example.com", password: "senha-publica")
    sign_in_as user
    ...
    expect(response).to have_http_status(:unprocessable_content)
    expect(response.body).to include("Esta é uma conta de demonstração. Estes dados de acesso são públicos e não podem ser alterados.")
    expect(user.reload.name).to eq("Ana")
  ...
  it "mantém a atualização Devise disponível para conta não-demo" do
    user = create(:user, name: "Bia", email: "bia-#{SecureRandom.hex(4)}@example.com", password: "senha-segura")
    sign_in_as user
    ...
    expect(user.reload.name).to eq("Beatriz")
    expect(user.email).to eq("bia.nova@example.com")
  end

  it "apresenta as credenciais demo como imutáveis sem formulário de edição" do
    user = create(:user, :demo_account, name: "Ana", email: "ana-demo-#{SecureRandom.hex(4)}@example.com")
    sign_in_as user
