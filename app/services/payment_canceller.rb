class PaymentCanceller < PaymentCommand
  def self.call(**attributes)
    new(**attributes).call
  end

  def initialize(group_id:, payment_id:, actor_user_id:, reason:, idempotency_key:)
    @group_id = group_id
  ...
  def call
    ...
    normalized_reason = normalize_reason!
    request_fingerprint = fingerprint([ "v1", "cancel", group_id, payment_id, actor_user_id, normalized_reason ])
    ...
    perform_idempotent_command(idempotency_key, request_fingerprint) do
      Group.transaction do
        ...
        raise Forbidden, "somente participantes cancelam" unless [ payment.from_user_id, payment.to_user_id ].include?(actor_user_id)
  ...
  attr_reader :group_id, :payment_id, :actor_user_id, :reason, :idempotency_key

  def normalize_reason!
    raise InvalidInput, "motivo inválido" unless reason.is_a?(String)

    normalized = reason.strip
    raise InvalidInput, "motivo inválido" if normalized.empty?
